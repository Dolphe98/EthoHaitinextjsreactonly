  ...
  return (
    <Link href={`/product/${product.slug}`} className="bg-white rounded-lg shadow hover:shadow-2xl transition-all duration-300 overflow-hidden group flex flex-col relative">
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        {/* MANAGER FIX: Split buttons 50/50 using flex-1 */}
        <div className="mt-auto flex gap-2 w-full">
          <button 
            onClick={handleAddToCart} 
            className={`flex-1 py-3 rounded font-bold text-sm text-white transition-colors shadow-sm flex items-center justify-center ${addedToCart ? 'bg-green-600' : 'bg-haitiRed hover:bg-red-700'}`}
          >
          ...
          <button 
            ...
            className="flex-1 py-3 bg-[#25D366] hover:bg-[#128C7E] flex items-center justify-center gap-1.5 rounded font-bold text-sm text-white transition-colors shadow-sm overflow-hidden" 
