...
export async function POST(request) {
  try {
    ...
    if (order.checkout_email?.toLowerCase() !== email.toLowerCase()) {
      return NextResponse.json({ error: "Unauthorized. The email provided does not match this order." }, { status: 403 });
    }
    ...
    const safeOrder = {
      ...
      shipping_address: {
        ...
        state: order.shipping_address?.state || order.shipping_address?.region || 'State hidden',
